...
export function elementoCarta(c,click){const el=document.createElement(click?"button":"div");el.className="card play-card";el.textContent=`${c?.valor||"?"}${c?.naipe||""}`;el.dataset.cartaId=c?.id||`${c?.valor||"?"}${c?.naipe||""}`;el.dataset.valor=c?.valor||"";el.dataset.naipe=c?.naipe||"";if(["♥","♦"].includes(c?.naipe))el.classList.add("red");if(click){el.type="button";el.classList.add("clickable");el.onclick=click;}return el;}
